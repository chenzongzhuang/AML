$(function() {
	//1.初始化Table
	var oTable = new omNewCRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var omNewCRatDataTable = function() {
	var omNewCRatDataTable = new Object();
	//初始化Table
	omNewCRatDataTable.Init = function() {
		$('#omNewCRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#omExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#omNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new omNewCRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#omExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("omExcelForm").action="/downExcel";
				document.getElementById("omExcelForm").submit();
			}
		}
		
	});
});
$("#omNewCRatQry").click(function(){
	//1.初始化Table
	$('#omNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new omNewCRatDataTable();
	oTable.Init();
});
$("#omNewCRatRefQry").click(function(){
	//1.初始化Table
	$('#omNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new omNewCRatDataTable();
	oTable.Init();
