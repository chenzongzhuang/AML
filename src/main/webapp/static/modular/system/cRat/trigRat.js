$(function() {
	//1.初始化Table
	var oTable = new trigRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var trigRatDataTable = function() {
	var trigRatDataTable = new Object();
	//初始化Table
	trigRatDataTable.Init = function() {
		$('#trigRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#trigRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#trigRatDataTable').bootstrapTable('destroy');  
	var oTable = new trigRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#trigRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("trigRatExcelForm").action="/downExcel";
				document.getElementById("trigRatExcelForm").submit();
			}
		}
		
	});
});
$("#trigRatQry").click(function(){
	//1.初始化Table
	$('#trigRatDataTable').bootstrapTable('destroy');  
	var oTable = new trigRatDataTable();
	oTable.Init();
});
$("#trigRatRefQry").click(function(){
	//1.初始化Table
	$('#trigRatDataTable').bootstrapTable('destroy');  
	var oTable = new trigRatDataTable();
	oTable.Init();
