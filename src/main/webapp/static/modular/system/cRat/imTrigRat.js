$(function() {
	//1.初始化Table
	var oTable = new imTrigRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var imTrigRatDataTable = function() {
	var imTrigRatDataTable = new Object();
	//初始化Table
	imTrigRatDataTable.Init = function() {
		$('#imTrigRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#imTrigRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#imTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new imTrigRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#imTrigRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("imTrigRatExcelForm").action="/downExcel";
				document.getElementById("imTrigRatExcelForm").submit();
			}
		}
		
	});
});
$("#imTrigRatQry").click(function(){
	//1.初始化Table
	$('#imTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new imTrigRatDataTable();
	oTable.Init();
});
$("#imTrigRatRefQry").click(function(){
	//1.初始化Table
	$('#imTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new imTrigRatDataTable();
	oTable.Init();
