$(function() {
	//1.初始化Table
	var oTable = new omTrigRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var omTrigRatDataTable = function() {
	var omTrigRatDataTable = new Object();
	//初始化Table
	omTrigRatDataTable.Init = function() {
		$('#omTrigRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#omTrigRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#omTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new omTrigRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#omTrigRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("omTrigRatExcelForm").action="/downExcel";
				document.getElementById("omTrigRatExcelForm").submit();
			}
		}
		
	});
});
$("#omTrigRatQry").click(function(){
	//1.初始化Table
	$('#omTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new omTrigRatDataTable();
	oTable.Init();
});
$("#omTrigRatRefQry").click(function(){
	//1.初始化Table
	$('#omTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new omTrigRatDataTable();
	oTable.Init();
