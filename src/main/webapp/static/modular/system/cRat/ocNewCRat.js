$(function() {
	//1.初始化Table
	var oTable = new ocNewCRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var ocNewCRatDataTable = function() {
	var ocNewCRatDataTable = new Object();
	//初始化Table
	ocNewCRatDataTable.Init = function() {
		$('#ocNewCRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#ocExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#ocNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocNewCRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#ocExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("ocExcelForm").action="/downExcel";
				document.getElementById("ocExcelForm").submit();
			}
		}
		
	});
});
$("#ocNewCRatQry").click(function(){
	//1.初始化Table
	$('#ocNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocNewCRatDataTable();
	oTable.Init();
});
$("#ocNewCRatRefQry").click(function(){
	//1.初始化Table
	$('#ocNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocNewCRatDataTable();
	oTable.Init();
