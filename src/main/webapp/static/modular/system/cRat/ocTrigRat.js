$(function() {
	//1.初始化Table
	var oTable = new ocTrigRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var ocTrigRatDataTable = function() {
	var ocTrigRatDataTable = new Object();
	//初始化Table
	ocTrigRatDataTable.Init = function() {
		$('#ocTrigRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#ocTrigRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#ocTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocTrigRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#ocTrigRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("ocTrigRatExcelForm").action="/downExcel";
				document.getElementById("ocTrigRatExcelForm").submit();
			}
		}
		
	});
});
$("#ocTrigRatQry").click(function(){
	//1.初始化Table
	$('#ocTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocTrigRatDataTable();
	oTable.Init();
});
$("#ocTrigRatRefQry").click(function(){
	//1.初始化Table
	$('#ocTrigRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocTrigRatDataTable();
	oTable.Init();
