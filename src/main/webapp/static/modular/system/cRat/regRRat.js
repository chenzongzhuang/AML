$(function() {
	//1.初始化Table
	var oTable = new regRRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var regRRatDataTable = function() {
	var regRRatDataTable = new Object();
	//初始化Table
	regRRatDataTable.Init = function() {
		$('#regRRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#regRRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#regRRatDataTable').bootstrapTable('destroy');  
	var oTable = new regRRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#regRRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("regRRatExcelForm").action="/downExcel";
				document.getElementById("regRRatExcelForm").submit();
			}
		}
		
	});
});
$("#regRRatQry").click(function(){
	//1.初始化Table
	$('#regRRatDataTable').bootstrapTable('destroy');  
	var oTable = new regRRatDataTable();
	oTable.Init();
});
$("#regRRatRefQry").click(function(){
	//1.初始化Table
	$('#regRRatDataTable').bootstrapTable('destroy');  
	var oTable = new regRRatDataTable();
	oTable.Init();
