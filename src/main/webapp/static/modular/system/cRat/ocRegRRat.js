$(function() {
	//1.初始化Table
	var oTable = new ocRegRRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var ocRegRRatDataTable = function() {
	var ocRegRRatDataTable = new Object();
	//初始化Table
	ocRegRRatDataTable.Init = function() {
		$('#ocRegRRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#ocRegRRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#ocRegRRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocRegRRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#ocRegRRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("ocRegRRatExcelForm").action="/downExcel";
				document.getElementById("ocRegRRatExcelForm").submit();
			}
		}
		
	});
});
$("#ocRegRRatQry").click(function(){
	//1.初始化Table
	$('#ocRegRRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocRegRRatDataTable();
	oTable.Init();
});
$("#ocRegRRatRefQry").click(function(){
	//1.初始化Table
	$('#ocRegRRatDataTable').bootstrapTable('destroy');  
	var oTable = new ocRegRRatDataTable();
	oTable.Init();
