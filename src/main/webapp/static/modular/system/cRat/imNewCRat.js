$(function() {
	//1.初始化Table
	var oTable = new imNewCRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var imNewCRatDataTable = function() {
	var imNewCRatDataTable = new Object();
	//初始化Table
	imNewCRatDataTable.Init = function() {
		$('#imNewCRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#imExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#imNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new imNewCRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#imExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("imExcelForm").action="/downExcel";
				document.getElementById("imExcelForm").submit();
			}
		}
		
	});
});
$("#imNewCRatQry").click(function(){
	//1.初始化Table
	$('#imNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new imNewCRatDataTable();
	oTable.Init();
});
$("#imNewCRatRefQry").click(function(){
	//1.初始化Table
	$('#imNewCRatDataTable').bootstrapTable('destroy');  
	var oTable = new imNewCRatDataTable();
	oTable.Init();
