$(function() {
	//1.初始化Table
	var oTable = new imRegRRatDataTable();
	oTable.Init();
	 //2.初始化Button的点击事件
...
//初始化Table
var imRegRRatDataTable = function() {
	var imRegRRatDataTable = new Object();
	//初始化Table
	imRegRRatDataTable.Init = function() {
		$('#imRegRRatDataTable').bootstrapTable({
...
//jxl生成excel并下载
$("#imRegRRatExportExcel").click(function(){
	//window.open('/downLoadTemp','文件下载窗口','height=300,width=450,left='+(screen.availWidth-400)/2+',top'+(screen.availHeight-300)/2);
	$('#imRegRRatDataTable').bootstrapTable('destroy');  
	var oTable = new imRegRRatDataTable();
	oTable.Init();
	$.ajax({
		url:'/produceExcel',
		dataType:'json',
		data:$("#imRegRRatExcelForm").serialize(),
		success:function(data){
			if(data.data.result=="success"){
				document.getElementById("imRegRRatExcelForm").action="/downExcel";
				document.getElementById("imRegRRatExcelForm").submit();
			}
		}
		
	});
});
$("#imRegRRatQry").click(function(){
	//1.初始化Table
	$('#imRegRRatDataTable').bootstrapTable('destroy');  
	var oTable = new imRegRRatDataTable();
	oTable.Init();
});
$("#imRegRRatRefQry").click(function(){
	//1.初始化Table
	$('#imRegRRatDataTable').bootstrapTable('destroy');  
	var oTable = new imRegRRatDataTable();
	oTable.Init();
